...
- `开始`：默认主路径，只需要填写任务，然后选择“先看方案”或“开始执行”
- `执行中`：运行态页面，支持 `实时态势 / 事件流 / 交付摘要` 三个子视图
- `历史结果`：查看历史 session、最终 summary、apply 结果与风险信息，并基于已完成 session 继续反馈

默认模式下，系统会自动使用当前配置或内置默认值处理协作模板、并发数和落地策略。  
只有在你按 `a` 展开“高级设置”后，才需要接触这些工程参数。

### 第 1 步：在“开始”页输入任务

按键：

- `1`~`3`：切换主页面；如果当前有任务正在执行，系统会强制留在“执行中”页
- `e` 或 `Enter`：编辑任务描述
- `m`：切换任务强度
- `a`：在“开始”页展开 / 收起高级设置；在其它空闲页面会先返回“开始”页再展开
- `↑` / `↓`：在高级设置里切换字段
- `←` / `→`：切换枚举字段
- `Esc`：任务编辑时保存退出；高级设置已展开时收起高级设置
- `c`：在“历史结果”页基于当前选中的 session 继续优化

任务编辑时还支持：

- `Ctrl+P`：保存任务并直接进入 plan
- `Ctrl+R`：保存任务并直接进入 run
...
如果你只是演示产品能力，保持默认即可。  
只有在需要控制并发、角色模板、结果落地策略时，再按 `a` 展开高级设置。
...
V6 推荐你把一次 `plan` / `run` 看作当前可评审版本，而不是终点。

操作方式：

1. 按 `3` 进入 `历史结果`
2. 用 `↑ / ↓` 选中一个已完成 session
3. 如有需要，用 `← / →` 切换 continue mode：`auto / plan / run`
4. 按 `c`
5. 如果还没写反馈，TUI 会先弹出“继续反馈”编辑框
6. 写完后按 `Esc` 保存，或者直接 `Ctrl+R`
7. 再按 `c`，系统会基于当前 session 启动下一轮 plan / run

这个入口适合处理：

- “第一版可以了，但我要再收敛一下方案”
- “结果基本对了，再把验证补完整”
- “保留当前实现，再优化这里的交互 / 文案 / 风险说明”
- “不要推倒重来，基于上一轮继续打磨”
