...
- `plan` 完成后，可以继续给反馈，把方案收敛成 `V2 / V3 / ...`
- `run` 完成后，可以继续补充“方向不对 / 粒度不够 / 验证不足 / 再优化这里”
- 每一轮都会保留新的 session、反馈记录、lineage 和迭代摘要
- 不覆盖旧结果，便于回放、比较、审查和回滚

这使得 `codex-forge` 更像一个**持续协作系统**，而不是“一次性跑完”的流水线。
...
> 从 v5 开始，直接执行 `codex-forge` 会默认进入终端产品主界面，而不是要求你先输入某个子命令。V6 在这个基础上继续增加了“继续反馈 / 继续迭代”的闭环入口。
...
如果不传 `--target-dir`，则默认使用你启动 `codex-forge` 时所在的仓库；如果当前目录只是仓库子目录，会自动归一到 Git 根目录。
...
如果你更喜欢命令行，而不是 TUI，可以直接使用：

```bash
codex-forge continue \
  --session <session-id> \
  --feedback "把第一版计划再收敛一下，突出下一步优化项"
```
...
每次 `continue` 都会生成新 session，并新增这些工件：

- `feedback.json` / `feedback.md`
- `session-lineage.json`
- `iteration-summary.md`
- 根 session 下的 `latest.md`

这保证了每一轮优化都**可追踪、可比较、可回放**。
...
```bash
# 回退指定 session 及其后续迭代落地的自动提交，并删除对应历史
codex-forge reset --session <session-id>

# 清理指定 session；如果后续迭代基于它继续推进，也会一起删除
codex-forge clean --session <session-id>

# 一键清空当前仓库下整个 .codex-forge
codex-forge clean --all
```

TUI 中对应热键：

- 在“历史结果”页按 `z`：回退当前选中 session 及其后续迭代落地的自动提交，再删除对应 `.codex-forge` 历史
- 在“历史结果”页按 `x`：清理当前选中的 session 及其后续迭代
- 在“历史结果”页按 `X`：一键清空当前仓库下整个 `.codex-forge`
- 三种操作都会先弹确认框，避免误删
- `reset` 只会在目标 session 的自动提交仍位于当前 `HEAD` 尾部时执行，避免误删后续人工提交
...
- 运行中按 `Esc` 不会离开执行页，而是提示你可按 `s` 停止
- 执行结束后按 `Esc` 会返回上一级：通常是“开始”页，若从“历史结果”页进入 replay，则会回到“历史结果”页
...
- 按 `3` 进入“历史结果”页
- 用 `↑` / `↓` 预览历史 session
- 按 `Enter` 打开当前 session 的结果详情
- 按 `l` 回放选中的 session
- 按 `z` 一键重置当前选中的 session 及其后续迭代
- 按 `x` 清理当前选中的 session 及其后续迭代
- 按 `X` 一键清空当前仓库下整个 `.codex-forge`
- 按 `Esc` 返回上一级
...
如果你想把 TUI 的演示黄金路径从“手工按键”变成“一键复走”，可以直接运行：
...
- 先执行 `cargo build --quiet`
- 创建临时 Git 仓库与 `fake codex`
- 用真实 PTY 启动 `codex-forge tui --target-dir <temp-repo>`
- 自动完成 `Start -> Doctor -> Plan -> Run -> History -> Replay`
- 对每个关键页面和结束态做屏幕断言，失败时保留 transcript 便于排障

如果你想保留临时目录与 PTY transcript，可加：

```bash
python3 scripts/tui_smoke_real_pty.py --keep-temp
```
...
- `--target-dir <path>`：目标仓库路径；不传时默认使用当前所在仓库，子目录会自动归一到 Git 根
...
9. 切到“历史结果”页，选择一个 session，按 `l` 做 replay
10. 在 replay 结束后按 `Esc` 回到“历史结果”页
11. 最后查看同一条 session 的交付摘要
